...
function get_zoom_custom_logo( $blog_id = 0 ) {
	...
	if ( $custom_logo_id ) {
		$custom_logo_attr = array(
            'class'    => 'custom-logo no-lazyload no-lazy a3-notlazy',
			'itemprop' => 'logo',
		);

		$info = zoom_customizer_logo_information();

		$width  = ( absint( get_theme_mod( 'custom_logo_size', '100' ) ) / 100 ) * absint( $info['width'] );
		$height = ( absint( get_theme_mod( 'custom_logo_size', '100' ) ) / 100 ) * absint( $info['height'] );
...
if ( ! function_exists( 'zoom_customizer_get_font_stack' ) ) :
	 ...
	 */
	function zoom_customizer_get_font_stack( $font, $default = '' ) {
		$fonts          = zoom_customizer_get_all_fonts();
...
if ( ! function_exists( 'zoom_customizer_get_popular_google_fonts' ) ) :
	...
	function zoom_customizer_get_popular_google_fonts() {
		 ...
		 */
		$whitelist_popular_google_fonts = apply_filters(
			'zoom_customizer_get_popular_google_fonts',
			array(
				'Open Sans',
				'Montserrat',
                'Inter',
...
if ( ! function_exists( 'zoom_get_google_font_uri' ) ) :
	...
	function zoom_get_google_font_uri( $data = array() ) {
		...
		$request     = 'https://fonts.googleapis.com/css';

		$fonts = array();

		if ( isset( $wp_customize ) ) {
			foreach ( $font_keys as $key => $custom_font ) {
				$fonts[] = $custom_font;
			}
		}
		...
		foreach ( $fonts as $font ) {
			...
			if ( array_key_exists( $font, $allowed_fonts ) ) {
				...
				$font_variants = isset( $variants[ $font ] ) ? zoom_customizer_choose_google_font_variants( $font, $variants[ $font ] ) : '';
...
if ( ! function_exists( 'zoom_customizer_get_google_font_used_variants' ) ) :
	function zoom_customizer_get_google_font_used_variants( $data, $available_variants ) {
				...
				if ( isset( $option['style']['rule'] ) && in_array( $option['style']['rule'], $rules ) ) {
					...
					if ( 'font-family' === $rule ) {
						...
						if ( $is_customize_preview ) {
							...
							if ( isset( $available_variants[ $font_family ] ) ) {
		...
		$output = apply_filters( 'wpzoom_get_google_font_variants', $output, $available_variants, $data );
...
if ( ! function_exists( 'zoom_customizer_get_google_fonts_from_api' ) ) :
	...
	function zoom_customizer_get_google_fonts_from_api() {
		...
		$api_key          = apply_filters( 'zoom_customizer_google_fonts_api_key', 'AIzaSyBjzHn-0kqpTfN4ZqMuTCTllqQXwAHMfqU' );
		
		static $transient = false;

		if ( empty( $transient ) ) {

			$transient = get_site_transient( 'zoom_customizer_google_fonts_json' );
			...
			if ( false === $transient ) {
				...
				// Check if we hit the rate limit or the API key is invalid
				if( 429 === $response_code || 400 === $response_code ) {
					...
					// Set the transient for 5 days (in seconds)
					$expiration_time = 5 * DAY_IN_SECONDS;
					
					set_site_transient( 'zoom_customizer_google_fonts_json', $transient, $expiration_time );
				}

				// Check if we have a valid response
				if ( 200 === $response_code && ! is_wp_error( $transient ) && ! empty( $transient ) ) {
					$decoded_transient = json_decode( $transient, true );
					if ( is_array( $decoded_transient ) && array_key_exists( 'items', $decoded_transient ) ) {
						
						// Set the transient for 3 months (in seconds)
						$expiration_time = 3 * MONTH_IN_SECONDS;
...
function zoom_customizer_get_font_familiy_ids( $data, $rule = 'font-family' ) {
	...
	$font_families = apply_filters( 'wpzoom_customizer_get_font_family_ids', $font_families, $rule, $data );
...
/**
 * Outputs the blog copyright text with support for custom placeholders.
 * This function extends the functionality of zoom_customizer_partial_blogcopyright()
 * to allow placeholder processing for greater flexibility.
 *
 * Although a similar function exists (zoom_customizer_partial_blogcopyright()),
 * this version was introduced for new functionality without breaking compatibility.
 */
function zoom_show_customizer_partial_blogcopyright() {
	$content = zoom_get_customizer_partial_blogcopyright_value();

	// Output the processed content with placeholders replaced.
	echo zoom_prepare_footer_copyright_text_tags( $content );
}

/**
 * Get the blog copyright value for display in the footer.
 * If the value is not set, use the default format with the current year and site title.
 *
 * @return string The blog copyright value.
 */
function zoom_get_customizer_partial_blogcopyright_value() {
	return get_option(
		'blogcopyright',
		'Copyright {copyright} {current-year} &mdash; {site-title}'
	);
}
 ...
 * - {current-year} -> current year (e.g., 1984 :) )
 ...
 */
function zoom_prepare_footer_copyright_text_tags( $content ) {
...
if ( ! function_exists( 'zoom_customizer_get_google_font_subsets' ) ) :
	...
	function zoom_customizer_get_google_font_subsets() {
		...
		return apply_filters(
			'zoom_customizer_get_google_font_subsets',
			array(
				'all'          => __( 'All', 'wpzoom' ),
...
function zoom_customizer_data_add_stylekits( $data ) {
	...
	if ( ! empty( $stylekits ) ) {
		...
		$data['style-kits'] = array(
			'title'    => __( 'Style Kits', 'wpzoom' ),
			'priority' => 21,
			'options'  => array( 'style-kits-selector' => $options ),
		);
	}
...
function zoom_customizer_get_stylekit_data() {
	...
	$stylekits = apply_filters( 'wpzoom_customizer_stylekits', array() );
	$stylekit  = sanitize_text_field( $_POST['stylekit'] );

	if ( ! current_user_can( 'edit_theme_options' ) || empty( $stylekits ) || ! isset( $stylekit ) || ! isset( $stylekits[ $stylekit ] ) ) {
...
function zoom_customizer_get_options_defaults() {
	...
	wp_send_json_success(
		array(
			'success' => true,
			'data'    => option::getCustomizerJsOptionsDefaults(),
		)
	);
