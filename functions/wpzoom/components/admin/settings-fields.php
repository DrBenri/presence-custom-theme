...
class WPZOOM_Admin_Settings_Fields {
	...
	public function select_category_multi( $args ) {
		...
		if ( is_countable( $catids ) && count( $catids ) > 0 ) {
			foreach ( $catids as $key => $val ) {
				$out .= "<option value=\"$val\"";
	...
	public function select_site_layout( $args ) {
		extract( $args );
		...
		foreach ( $options as $key => $val ) {
			...
			$out .= '">';
			$out .= '<img src="' . WPZOOM::$themePath . "/functions/assets/image/site-layout_$key.png\" alt=\"\" title=\"$val\" class=\"layout-select\" /></label>";
		}
		$out .= "<p class=\"description\">$desc</p>";
	...
	public function upload( $args ) {
		...
		if ( $id === 'misc_favicon' && function_exists( 'has_site_icon' ) ) {
			$out .= sprintf(
				__( '<p>This option has been deprecated in favor of the Site Icon setting in WordPress core. Please visit %s to configure your site icon.</p>', 'wpzoom' ),
				sprintf(
					'<a href="%1$s">%2$s</a>',
