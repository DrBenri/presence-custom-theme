...
if ( is_admin() ) {
	...
	require_once WPZOOM_INC . '/components/guard-shell.php';

	require_once WPZOOM_INC . '/components/helpers.php';
 ...
 */
add_action( 'after_setup_theme', 'zoom_load_components', 11 );
...
function zoom_load_components() {
	...
	if ( ! in_array( WPZOOM::$theme_raw_name, $ignored_themes ) ) {
		...
		/**
		 * Because we extend the PHP class WPZOOM_Theme_Tour in onboarding, we need to create a class instance only when the theme doesn't support onboarding feature.
		 *
		 * @since 2.0.0
		 */
		if ( ! zoom_current_theme_supports( 'wpz-onboarding', 'onboarding-tour' ) ) {
			new WPZOOM_Theme_Tour();
		}

		if ( is_admin() ) {
			/**
			 * Include old demo importer only if current screen is 'wpzoom_options' and current theme doesn't support onboarding demo importer.
			 *
			 * @since 2.0.0
			 */
			...
			if ( zoom_current_theme_supports( 'wpz-onboarding' ) ) {
				require_once WPZOOM_INC . '/components/onboarding/class-wpzoom-onboarding.php';
				add_filter( 'wpzoom_demo_importer_display_button', '__return_false' );
			}
		}
	}
     ...
     */
    if ( option::is_on( 'disable_google_fonts_wpz' ) ) {
        require_once WPZOOM_INC . '/components/theme/google-fonts.php';
     ...
     */
    if ( option::is_on( 'enable_google_fonts_downloader' ) ) {
        require_once WPZOOM_INC . '/components/performance/class-wpzoom-google-fonts-downloader.php';
...
require_once WPZOOM_INC . '/components/tgmpa/class-wpzoom-tgmpa-assistance.php';
...
function zoom_register_theme_required_plugins() {
	...
	$plugins = array(
		array(
			...
			// Advanced feature. If a plugin can be installed under two or more different slugs - for instance a basic version and a premium version using different slugs -, it might not be recognized correctly as active.
			'is_callable' => array( 'WPZOOM_Social_Icons_License', 'instance' ),
		),

		array(
			'name'     => 'WPZOOM Forms', // The plugin name.
			'slug'     => 'wpzoom-forms', // The plugin slug (typically the folder name).
			'required' => false, // If false, the plugin is only 'recommended' instead of required.
		),
	);
	$plugins = WPZOOM_TGMPA_Assistance::get_instance()->parse_plugins( $plugins );
