...
class WPZOOM_Theme_Tour {
	...
	protected $pointer_close_id = 'wpzoom_theme_tour'; // value can be cleared to retake tour
	...
	public function __construct() {
		...
		// Check the license validation
		if( !WPZOOM_Admin_Settings_Page::check_button() ) {
			return false;
		}
	...
	public function get_theme_tour_pointers() {
		...
		$adminpages = array(
			...
			'wpzoom_options_2' => array(
				'content' => '<h3>' . __( 'Need Help?', 'wpzoom' ) . '</h3><p><strong>' . __( 'Congratulations! Your theme is fully set up.', 'wpzoom' ) . '</strong></p><p>' . __( 'If you need help with the theme or have a question get in touch with our Support Team. We\'d love the opportunity to help you.', 'wpzoom' ) . '</p>',
				'id'      => 'zoomInfo-support',
			),
		);
		...
		if ( ! empty( $inactive_plugins ) ) {
			$adminpages['wpzoom_options'] = array(
				'content'  => '<h3>' . __( 'Install Recommended Plugins', 'wpzoom' ) . '</h3><p><strong>' . sprintf( __( 'We recommend you install and activate the required plugins for a better experience with the %s theme!', 'wpzoom' ), WPZOOM::$themeName ) . '</strong></p>',
		...
		if ( $demos['imported'] ) {
			$adminpages['wpzoom_options'] = array(
				'content'  => '<h3>' . __( 'Demo successfully imported', 'wpzoom' ) . '</h3><p><strong>' . sprintf( __( 'You have imported the «%1$s» demo content for the %2$s theme. Now we recommend goint to the Next step.', 'wpzoom' ), zoom_get_beauty_demo_title( $demos['imported'] ), WPZOOM::$themeName ) . '</strong></p>',
				'id'       => 'wpz-demo-content-icon',
				'button2'  => __( 'Next', 'wpzoom' ),
				'function' => 'window.location="' . admin_url( 'admin.php?page=wpzoom_options&welcome_tour=1' ) . '";',
			);
		...
		return apply_filters( 'wpzoom_theme_tour_pointers', $adminpages );
	}
	...
	public function intro_tour() {
		...
		$adminpages = $this->get_theme_tour_pointers();
	 ...
	 */
	protected function print_scripts( $selector, $options, $button1, $button2 = false, $button2_function = '', $button1_function = '' ) {
		?>
