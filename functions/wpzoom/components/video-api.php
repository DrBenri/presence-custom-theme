...
if(!class_exists('WPZOOM_Video_API')){
...
class WPZOOM_Video_API {
	...
	public static function extract_url_from_embed( $embed_code ) {
		...
		$DOM = new DOMDocument();
		if ( $DOM->loadHTML( $embed_code ) === false ) {
			return false;
		}

		$iframes = $DOM->getElementsByTagName( 'iframe' );
		if ( empty( $iframes ) || $iframes->length < 1 ) {
			return false;
		}

		$iframe = $iframes->item( 0 );
		if ( $iframe == null || ! $iframe->hasAttributes() ) {
			return false;
		}
	...
	public static function convert_embed_url( $url ) {
		...
		if ( $url_parts === false || empty( $url_parts ) || ! is_array( $url_parts ) ||
		   ...
		   ! isset( $url_parts['path'] ) || empty( $url_parts['path'] ) ) {
			return false;
		}
	...
	public static function extract_video_id( $url ) {
		...
		return ! empty( $provider ) && ! empty( $id ) ? array(
			'id'       => $id,
			'provider' => $provider,
		) : false;
	}
