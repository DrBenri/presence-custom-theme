...
class WPZOOM_Customizer_Controls {
	...
	public function enqueue_control_scripts() {
		...
		wp_localize_script(
			...
			'WPZOOM_Customizer',
			WPZOOM_Setup_Customizer::set_restricted_customizer_data()
		);
	}
	...
	public function setup_control_types( WP_Customize_Manager $wp_customize ) {
		...
		$types = array(
			...
			'WPZOOM_Customizer_Control_Copyright_WP_Editor'
		);
	...
	private function get_custom_controls() {
		...
		return apply_filters(
			'wpzoom_customizer_get_custom_controls',
			array(
	...
	public function add_sections( WP_Customize_Manager $wp_customize ) {
		...
		foreach ( $this->get_sections() as $section => $data ) {
            ...
            // Set the type so we know it is a WPZOOM section
            $data['type'] = 'wpzoom';

            // Set an icon class if needed
            if ( isset( $data['wpzoom_icon'] ) ) {
                $data['type'] .= ' wpzoom-section-with-icon wpzoom-section-icon_' . esc_attr( $data['wpzoom_icon'] );
            }
