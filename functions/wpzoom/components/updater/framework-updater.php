...
class WPZOOM_Framework_Updater {
			...
			$data = array(
				'message' => '
                        <div class="popup-modal-message success-message">
                            ...
                            <p class="description">' . __( 'You have successfully updated WPZOOM Framework!', 'wpzoom' ) . '</p>
                        </div>',
			);

			if ( $action == 'update' ) {
				$fwUrl  = 'https://framework.wpzoom.com/wpzoom-framework.zip';
	...
	public static function get_remote_version() {
		...
		$url = 'https://framework.wpzoom.com/get-version/';
	...
	public static function get_changelog() {
		...
		$url = 'https://framework.wpzoom.com/changelog/';
	...
	public function admin_notification() {
		...
		if ( 'wpzoom_page_wpzoom_update' == $current_page ) {
			return;
		}

		$msg = sprintf( __( '<p>A new version of WPZOOM Framework is available. Please <a href="%s">update now</a>. <input type="button" class="close button" value="Dismiss" /></p>', 'wpzoom' ), admin_url( 'admin.php?page=wpzoom_update' ) );
	...
	public static function check_license_status() {
		...
		$license_status = get_option( WPZOOM::$theme_raw_name . '_license_key_status' );
		
		// If the license is valid, inactive or site_inactive, we can update the framework
		if( 'valid' == $license_status || 'inactive' == $license_status || 'site_inactive' == $license_status ) {
			return true;
		}
		
		// If the license is expired, suspended or disabled, we can't update the framework
		return false;
