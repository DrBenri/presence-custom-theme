			...
			if ( $id == 'sidebars_widgets' ) {
				...
				if ( isset( $builder_widgets[ $widget_id ] ) ) {
					foreach ( $current_value as $widget_num => $widget_settings ) {
						if ( in_array( $widget_num, $builder_widgets[ $widget_id ] ) ) {
							$keep[ $widget_num ] = $widget_settings;
						}
					}
				}
			}
