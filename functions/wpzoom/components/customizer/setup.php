...
class WPZOOM_Setup_Customizer {
	...
	public static function set_restricted_customizer_data() {
		...
		$info_head    = '';
		$info_html    = '';
		$info_message = '<p>' . wp_kses_post( __( 'Your theme is not licensed for use on this website, or your license key has expired. <br/>Please activate or renew your license key to use this theme.', 'wpzoom' ) ) . '</p>';
		$info_btn     = sprintf( '<a href="%s" target="_blank" class="button button-primary">%s</a>', esc_url( admin_url( 'admin.php?page=wpzoom_license#license' ) ), esc_html__( 'Enter your License Key', 'wpzoom' ) );

		if ( ! empty( $license ) ) {
			if ( 'invalid' === $status ) {
				$info_head = '<h3>' . esc_html__( 'Invalid License Key!', 'wpzoom' ) . '</h3>';
			} elseif ( 'expired' === $status ) {
				$info_head = '<h3>' . esc_html__( 'Expired License Key!', 'wpzoom' ) . '</h3>';
			} elseif ( 'valid' !== $status ) {
				$info_head = '<h3>' . esc_html__( 'Inactive License Key!', 'wpzoom' ) . '</h3>';
			}
		} else {
			$info_head = '<h3>' . esc_html__( 'Activate Your License Key!', 'wpzoom' ) . '</h3>';
		}

		if ( ! empty( $info_head ) ) {
			$info_html = $info_head . $info_message . $info_btn;
			$data['is_limited'] = true;
			$data['message'] = $info_html;
		}
