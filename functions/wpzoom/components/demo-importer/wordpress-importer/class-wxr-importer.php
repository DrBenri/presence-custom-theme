...
class WXR_Importer extends WP_Importer {
			...
			switch ( $reader->name ) {
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning(
							sprintf(
								__( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'wpzoom' ),
								$this->version,
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			$this->logger->warning(
				sprintf(
					__( 'Failed to import "%1$s": Invalid post type %2$s', 'wpzoom' ),
					$data['post_title'],
					$data['post_type']
				)
			);
		...
		if ( $post_exists ) {
			$this->logger->info(
				sprintf(
					__( '%1$s "%2$s" already exists.', 'wpzoom' ),
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error(
				sprintf(
					__( 'Failed to import "%1$s" (%2$s)', 'wpzoom' ),
					$data['post_title'],
					$post_type_object->labels->singular_name
				)
			);
		...
		$this->logger->debug(
			sprintf(
				__( 'Post %1$d remapped to %2$d', 'wpzoom' ),
				$original_id,
				$post_id
			)
		);
	...
	protected function process_author( $data, $meta ) {
		...
		$this->logger->debug(
			sprintf(
				__( 'User %1$d remapped to %2$d', 'wpzoom' ),
				$original_id,
				$user_id
			)
		);
	...
	protected function process_term( $data, $meta ) {
		...
		$this->logger->debug(
			sprintf(
				__( 'Term %1$d remapped to %2$d', 'wpzoom' ),
				$original_id,
				$term_id
			)
		);
		...
		foreach ( $todo as $post_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the post parent for "%1$s" (post #%2$d)', 'wpzoom' ),
					...
					$this->logger->debug(
						sprintf(
							__( 'Post %1$d was imported with parent %2$d, but could not be found', 'wpzoom' ),
							$post_id,
							$parent_id
						)
					);
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning(
						sprintf(
							__( 'Could not find the author for "%1$s" (post #%2$d)', 'wpzoom' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
					$this->logger->debug(
						sprintf(
							__( 'Post %1$d was imported with author "%2$s", but could not be found', 'wpzoom' ),
							$post_id,
							$author_slug
						)
					);
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning(
					sprintf(
						__( 'Could not update "%1$s" (post #%2$d) with mapped data', 'wpzoom' ),
						get_the_title( $post_id ),
						$post_id
					)
				);
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning(
				sprintf(
					__( 'Could not find the menu object for "%1$s" (post #%2$d)', 'wpzoom' ),
					get_the_title( $post_id ),
					$post_id
				)
			);
			$this->logger->debug(
				sprintf(
					__( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'wpzoom' ),
					$post_id,
					$menu_object_id,
					$menu_item_type
				)
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->debug(
						sprintf(
							__( 'Comment %1$d was imported with parent %2$d, but could not be found', 'wpzoom' ),
							$comment_id,
							$parent_id
						)
					);
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->debug(
						sprintf(
							__( 'Comment %1$d was imported with author %2$d, but could not be found', 'wpzoom' ),
							$comment_id,
							$author_id
						)
					);
