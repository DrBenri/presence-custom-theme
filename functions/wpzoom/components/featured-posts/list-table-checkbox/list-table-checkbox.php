...
class WPZOOM_List_Table_Checkbox_Option_Type {
	...
	public function __construct( $option, $component_uri ) {
		...
		add_filter(
			'manage_edit-' . $this->option['post_type'] . '_columns',
			array(
				$this,
				'get_column_header',
			),
		...
		add_action(
			'manage_' . $this->option['post_type'] . '_posts_custom_column',
			array(
				$this,
				'get_column_content',
			),
	...
	public function get_menu_order_limit( $post_type, $limit = 'MIN' ) {
		global $wpdb;
	...
	public function ajax_callback() {
		if ( ! empty( $_POST['post_id'] ) &&
			...
			$max_order = $this->get_menu_order_limit( $post_type, 'MAX' );
