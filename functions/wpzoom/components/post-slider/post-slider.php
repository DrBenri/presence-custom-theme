...
class ZOOM_Post_Slider {
	...
	public function show_metabox( $post, $metabox ) {
		...
		if ( ! empty( $meta ) ) {
			foreach ( $meta as $item ) {
				...
				$description = isset( $item['description'] ) && ! empty( $item['description'] ) ? wp_kses_post( $item['description'] ) : '';
	...
	public function save_post( $post_id ) {
		...
		foreach ( $slides as $slide ) {
			...
			$caption     = isset( $slide['caption'] ) && ! empty( $slide['caption'] ) ? esc_html( $slide['caption'] ) : '';
			$description = isset( $slide['description'] ) && ! empty( $slide['description'] ) ?  wp_kses_post( $slide['description'] ) : '';
			...
			if ( ! empty( $caption ) ) {
				$new_arr['caption'] = $caption;
			}
			if ( ! empty( $description ) ) {
				$new_arr['description'] = $description;
			}

			if ( ! isset( $new_arr['imageId'] ) && ! isset( $new_arr['embedCode'] ) && ! isset( $new_arr['caption'] ) && ! isset( $new_arr['description'] ) ) {
				continue;
			}
	...
	public function assets( $hook ) {
		...
		if ( isset( $typenow ) && ! array_key_exists( $typenow, $this->sliders ) ) {
			return;
		}
