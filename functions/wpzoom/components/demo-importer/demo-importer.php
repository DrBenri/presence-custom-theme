...
class ZOOM_Demo_Importer {
	...
	public function demo_importer_options( $data ) {
		...
		$desc .= '<p class="description nofloat" style="width:100%; margin-bottom:20px;">' . sprintf( __( 'If the importer doesn\'t work for you, try importing the demo content %1$smanually%2$s.', 'wpzoom' ), '<a href="https://www.wpzoom.com/theme-demo-content/" target="_blank">', '</a>' ) . '</p>';
		...
		$desc .= '<div class="clear"></div><div id="wpzoom-demo-content-info"><p>' .
				__(
					'Importing the Demo Content will not delete your current posts, pages or anything else. You can delete the demo content (posts, pages, menus) from your site by clicking <u>Erase content</u>.<br/><br/>
                    The importing process can take up to <strong>5 minutes</strong> or longer depending on your server configuration and the number of images included in the demo. Please do not leave this screen until you see &ldquo;<strong><em>All done!</em></strong>&rdquo; in the box that will appear below.',
					'wpzoom'
				) .
				'</p></div>';
		...
		$data['import-export'][] = array(
			'type' => 'preheader',
			'name' => __( 'Demo Content', 'wpzoom' ),
		);

		$data['import-export'][] = array(
			'name'  => __( 'Load Demo Content', 'wpzoom' ),
			'desc'  => $desc,
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'zoom-demo-importer', $this->get_js_uri( 'general.js' ), array( 'jquery', 'underscore' ), WPZOOM::$wpzoomVersion, true );
	...
	public function check_existing_demo() {
		...
		$response = array(
			...
			'message'  => '<div class="popup-modal-message warning-message">
                            ...
                            <p class="description">' . sprintf( __( 'Do you want to erase previous demo content (%1$s) and then import selected (%2$s) one? <strong>After the erase the importing process will start automatically!</strong>', 'wpzoom' ), zoom_get_beauty_demo_title( $this->imported_demo ), zoom_get_beauty_demo_title( $this->selected_demo ) ) . '</p>
	...
	public function get_modal_demo_importer_template() {
		...
		$tgmpa            = WPZOOM_TGMPA_Assistance::get_instance()->tgmpa_instance();
		$inactive_plugins = WPZOOM_TGMPA_Assistance::get_instance()->get_inactive_plugins();
