...
class WPZOOM_Theme_Setup {
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'zoom-theme-setup', $this->get_js_uri( 'general.js' ), array( 'jquery', 'underscore', 'wp-util' ), WPZOOM::$wpzoomVersion, true );
		wp_enqueue_script( 'zoom-magnific-popup', $this->get_js_uri( 'jquery.magnific-popup.min.js' ), array(), WPZOOM::$wpzoomVersion );
		wp_enqueue_script( 'zoom-retry-ajax', $this->get_js_uri( 'jquery.ajax.retry.js' ), array(), WPZOOM::$wpzoomVersion );
	...
	public function get_theme_setup_options( $merge = array() ) {
		...
		$child_theme_file_exists = ZOOM_Child_Theme::instance()->child_theme_file_exists();
	...
	public function get_front_page_content() {
		...
		$out .= '<p class="description nofloat text-left">' . sprintf( __( 'You can also configure your front page and posts page later from the %1$sReading Settings%2$s', 'wpzoom' ), '<a href="' . admin_url( 'options-reading.php' ) . '" target="_blank" title="' . __( 'Open Reading settings page', 'wpzoom' ) . '">', '</a>' ) . '</p>';
	...
	public function regenerate_thumbnail() {
		...
		$thumb_id     = isset( $_POST['thumb_id'] ) ? sanitize_text_field( $_POST['thumb_id'] ) : 0;
		$fullsizepath = isset( $_POST['file'] ) ? sanitize_text_field( $_POST['file'] ) : false;
		...
		if ( ! $image || 'attachment' != $image->post_type || 'image/' != substr( $image->post_mime_type, 0, 6 ) ) {
			wp_send_json_error(
				array(
					...
					'image'    => $image,
					'heading'  => sprintf(
						__( '&quot;%1$s&quot; (ID %2$s) is Failed.', 'wpzoom' ),
		...
		if ( false === $fullsizepath || ! file_exists( $fullsizepath ) ) {
			wp_send_json_error(
				array(
					...
					'message'  => sprintf(
						__( 'The originally uploaded image file cannot be found at %s', 'wpzoom' ),
		...
		/**
		 * Pass $message to importer log
		 *
		 * @since 2.0.0
		 */
		if ( class_exists( 'WPZOOM_Demo_Importer_Log' ) ) {
			WPZOOM_Demo_Importer_Log::add( str_replace( '&quot;', '', $message ) );
		}
