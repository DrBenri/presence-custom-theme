...
class EDD_Theme_Updater_Admin {
	...
	function get_default_guide_data() {
		...
		return array(
			'documentation' => array(
				'header'  => __( 'Theme Documentation', 'wpzoom' ),
				'content' => __( '<strong>Theme Documentation</strong> is the place where you\'ll find the information needed to set up the theme quickly, and other details about theme-specific features.', 'wpzoom' ),
				'actions' => array(
...
',
			...
			'support'       => array(
				...
				'actions' => array(
					'<a class="button button-primary" href="https://www.wpzoom.com/support/tickets/" target="_blank">Open the Support Desk »</a>',
				),
			),
		);
	}
	...
	function license_notice() {
		...
		if ( $license_status == 'expired' ) {
			...
			echo sprintf( __( '<div class="wpz-notice-aside"><span class="wpz-circle wpz-pulse"> ! </span></div> <div class="wpz-notice-content"><p><strong>Your %1$s theme license has %2$s</strong>. Please renew it to continue receiving theme updates and support. <em><strong>Your website may be at security risk!</strong></em></p> <p>%3$sRenew your license key%4$s</p></div>', 'wpzoom' ), $wpz_theme_name, $license_status, '<a href="' . $this->get_renewal_link() . '"  target="_blank" class="button button-primary">', '</a>' );
			echo '</div>';
		} elseif ( $license_status == 'invalid' ) {
			echo '<div id="update-nag" class="notice notice-error settings-error wpz-error notice notice-error notice-alt">';
			echo sprintf( __( '<div class="wpz-notice-aside"><span class="wpz-circle wpz-pulse"> ! </span></div> <div class="wpz-notice-content"><p>If you want to receive updates for <strong>%1$s theme</strong>, please %2$senter your license key%3$s or %4$spurchase a new license%5$s if you don\'t have one yet.</p></div>', 'wpzoom' ), $wpz_theme_name, '<strong><a href="admin.php?page=wpzoom_license#license">', '</a></strong>', '<strong><a href="https://www.wpzoom.com/themes/' . WPZOOM::$theme_raw_name . '/" target="_blank">', '</a></strong>' );
			echo '</div>';
		} elseif ( $license_status == 'inactive' || $license_status == 'site_inactive' ) {
			echo '<div id="update-nag" class="notice notice-error settings-error wpz-error notice notice-error notice-alt">';
			echo sprintf( __( '<div class="wpz-notice-aside"><span class="wpz-circle wpz-pulse"> ! </span></div> <div class="wpz-notice-content"><p>Your <strong>%1$s theme</strong> license key is <strong>inactive</strong>. Please %2$sgo to this page%3$s and click the <strong>Activate License</strong> button to enable theme updates.</p></div>', 'wpzoom' ), $wpz_theme_name, '<strong><a href="admin.php?page=wpzoom_license#license">', '</a></strong>' );
			echo '</div>';
		} elseif ( $license_status == 'item_name_mismatch' ) {
			echo '<div id="update-nag" class="notice notice-error settings-error wpz-error notice notice-error notice-alt">';
			echo sprintf( __( '<div class="wpz-notice-aside"><span class="wpz-circle wpz-pulse"> ! </span></div> <div class="wpz-notice-content"><p>You have entered an incorrect license key for <strong>%1$s theme</strong>. Please %2$senter a correct key%3$s or %4$spurchase a new license%5$s for theme you\'re currently using.</p></div>', 'wpzoom' ), $wpz_theme_name, '<strong><a href="admin.php?page=wpzoom_license#license">', '</a></strong>', '<strong><a href="https://www.wpzoom.com/themes/' . WPZOOM::$theme_raw_name . '/" target="_blank">', '</a></strong>' );
			echo '</div>';
		} elseif ( $license_status == 'disabled' ) {
			echo '<div id="update-nag" class="notice notice-error settings-error wpz-error notice notice-error notice-alt">';
			echo sprintf( __( '<div class="wpz-notice-aside"><span class="wpz-circle wpz-pulse"> ! </span></div> <div class="wpz-notice-content"><p>Your <strong>%1$s theme</strong> license key is <strong>%2$s</strong>, please %3$scheck the status%4$s of your license or get in touch with the WPZOOM team for more details.</p></div>', 'wpzoom' ), $wpz_theme_name, $license_status, '<strong><a href="' . $this->get_renewal_link() . '">', '</a></strong>' );
			echo '</div>';
		} elseif ( $license_status != 'valid' ) {
			echo '<div id="update-nag" class="notice notice-error settings-error wpz-error notice notice-error notice-alt">';
			echo sprintf( __( '<div class="wpz-notice-aside"><span class="wpz-circle wpz-pulse"> ! </span></div> <div class="wpz-notice-content"><p>Your <strong>%1$s theme</strong> license is <strong>%2$s</strong>, please %3$sactivate your license key%4$s to enable theme updates.</p></div>', 'wpzoom' ), $wpz_theme_name, $license_status, '<strong><a href="' . $this->get_renewal_link() . '">', '</a></strong>' );
			echo '</div>';
		}
	}
	...
	function admin_styles() {
		wp_enqueue_style(
			...
			WPZOOM::get_root_uri() . '/components/theme-updater/admin.css',
			array(),
	...
	function license_page() {
		...
		<div class="cols-wrap">
			...
			<?php if( WPZOOM_Guard_Shell::is_guard_active() ) : ?>

				<div class="license-wrap wpz_license_column">
					<h2 class="headline"><?php _e( 'Activate Your License Key', 'wpzoom' ); ?></h2>
					<p>
						<?php
						echo sprintf( __( 'Enter your license key to enable <strong>theme updates</strong>. You can find your license key on the <a href="https://www.wpzoom.com/account/licenses/" target="_blank">Licenses</a> page.', 'wpzoom' ) );
						?>
					</p>
					<form method="post" action="options.php">
						...
						<div>
							<input id="<?php echo esc_attr( $this->theme_slug . '_license_key' ); ?>" name="<?php echo esc_attr( $this->theme_slug . '_license_key' ); ?>" type="password" class="regular-text" value="<?php echo esc_attr( $license ); ?>" />
						</div>
						<?php
				...
				<?php else: ?>
					<div class="license-wrap wpz_license_column license-retired">
						<h3 class="headline"><?php esc_html_e( 'Your theme is outdated!', 'wpzoom' ); ?></h3>
						<p><?php esc_html_e( 'You\'re using a discontinued theme. We highly recommend switching to a different theme compatible with the latest WordPress updates.', 'wpzoom' ); ?></p>
						<a class="cta-button" href="https://www.wpzoom.com/themes/" target="_blank"><?php esc_html_e( 'Get a New Theme &rarr;', 'wpzoom' ); ?></a>
					</div>
			...
			<div class="theme-guide">
				...
				<p class="description"><?php _e( 'Just installed a new theme and not sure where to start? We\'re here to help!', 'wpzoom' ); ?></p>
					...
					<ol>
						...
						<li>
							...
							<?php if ( ! empty( $guide_line['actions'] ) ) : ?>
								<?php
								foreach ( (array) $guide_line['actions'] as $action ) {
	...
	public function activate_license() {
			...
			if ( false === $license_data->success ) {
				switch ( $license_data->error ) {
					...
					case 'no_activations_left':
						$message = sprintf( __( '<strong>Your theme license key has reached its activation limit. Please %1$supgrade%2$s your license or contact WPZOOM if you have an unlimited license.</strong>', 'wpzoom' ), '<a href="' . esc_url( 'https://www.wpzoom.com/account/licenses/' ) . '" target="_blank">', '</a>' );
						break;
				...
				if ( ! empty( $message ) ) {
					$base_url = admin_url( 'admin.php?page=wpzoom_license' );
		...
		wp_redirect( admin_url( 'admin.php?page=wpzoom_license' ) );
		exit();
	}
	...
	public function get_renewal_link() {
		...
		if ( '' !== $license_key ) {
			...
			$url .= 'checkout/?edd_license_key=' . urlencode( $license_key );
			return $url;
		}
